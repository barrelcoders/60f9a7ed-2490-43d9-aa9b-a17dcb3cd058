...
<?php
	...
	$rsHeaderDetial=mysql_query("select `srno`,`HeadName`,`HeadAmount`,`sclass`,`LastDate`,`Remarks`,(SELECT `HeadType` FROM  `fees_misc_head` where `HeadName`=a.`HeadName`) as `HeadType`,`AnnouncementID` from `fees_misc_announce` as `a` where `sclass`='$StudentClass' and `srno`='$HeaderSrNo'");
	while($row=mysql_fetch_row($rsHeaderDetial))
	{
		$HeadName=$row[1];
		$HeadAmount=$row[2];
		$sclass=$row[3];
