...
if ($txnid != "")
{
	//$AdmissionNo=$_REQUEST["txtAdmissionNo"];
		...
		while($row = mysql_fetch_row($rsRcpt))
				{
					$newsrno=$row[0];
					$NewReciptNo="HR/2015-2016/" . $newsrno;
					//$NewReciptNo=$newsrno;
				}
		...
		while($rows = mysql_fetch_row($rsStudentDetail))
		{
			...
			$ssql2="INSERT INTO `fees_student` (`sadmission`,`class`,`Name`,`feeshead`,`amount`,`financialyear`,`FeesType`) VALUES ('$sadmission','$sclass','$sname','$FeeHead','$amount','$FinancialYear','Hostel')";
