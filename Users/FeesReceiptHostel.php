...
if ($txnid != "")
{
			...
			$ssql="UPDATE fees_hostel_temp SET `TxnStatus`='$txnstatus',`PGTxnId`='$pgtxnno' where `TxnId`='$txnid'";
		...
		$sqlRcpt = "select max(x.`srno`) +1 from (SELECT max(CONVERT(replace(`receipt`,'HR/".$CurrentFinancialYear."/',''),UNSIGNED INTEGER)) as `srno` FROM fees_hostel) as `x`";
		...
		while($row = mysql_fetch_row($rsRcpt))
				{
					$newsrno=$row[0];
					$NewReciptNo="HR/".$CurrentFinancialYear."/" . $newsrno;
					//$NewReciptNo=$newsrno;
				}
		$PDFFileName=$NewReciptNo .".pdf";
		
		$ssql="UPDATE fees_hostel_temp SET `receipt`='$NewReciptNo' where `TxnId`='$txnid'";
		mysql_query($ssql) or die(mysql_error());
			...
			$currentYear=date("Y");
					...
					$rsChk=mysql_query("select `quarter`,`FinancialYear`,`sadmission` from fees_hostel_temp where `TxnId`='$txnid'");
		...
		while($rows = mysql_fetch_row($rsStudentDetail))
		{
			...
			break;
					...
					$sstr="select * from fees_hostel where `sadmission`='$sadmission' and `quarter`='$quarter' and `FinancialYear`='$FinancialYear' and `FeesType` ='Hostel'";
					$rs = mysql_query($sstr);
			...
			$ssql2="INSERT INTO `fees_student_hostel` (`sadmission`,`class`,`Name`,`feeshead`,`amount`,`financialyear`,`FeesType`) VALUES ('$sadmission','$sclass','$sname','$FeeHead','$amountpaid','$FinancialYear','Hostel')";
...
//-------------------- Previous Payment history----------------------------------------------------------
	$ssql = "SELECT `quarter`,`fees_amount`,`amountpaid`,`BalanceAmt`,`status`,`receipt`,date_format(`date`,'%d-%m-%Y') as `date`,`FinancialYear` FROM fees_hostel where `sadmission`='$AdmissionNo' and `FeesType`='Hostel' order by `quarter`,`FinancialYear` desc limit 4";
	$rs = mysql_query($ssql);
...
?>
	...
	<tr>
		...
		<span >Late fees fine of Rs 10/- per day will charged &amp; will be applicable from the 10th Day of 
		first month of every 

		quarter. </span>For any queries, Kindly call at 
