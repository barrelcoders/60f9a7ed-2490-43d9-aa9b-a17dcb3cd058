...
if ($txnid != "")
{
		...
		$rstxnDetail=mysql_query("select `sadmissionno`,`sname`,`sclass`,`srollno` from `fees_misc_collection_tmp` where `TxnId`='$txnid'");
		while($row=mysql_fetch_row($rstxnDetail))
		{
			$sadmissionno=$row[0];
			$sname=$row[1];
			$sclass=$row[2];
		...
		mysql_query($ssql) or die(mysql_error());
			...
			{
				mysql_query("insert into fees_misc_collection (`date`,`AnnouncementID`,`HeadName`,`sadmissionno`,`sname`,`sclass`,`srollno`,`Amount`,`PaymentMode`,`TxnId`,`TxnStatus`,`PGTxnId`,`FeeReceipt`,`HeadType`,`financialyear`) select `date`,`AnnouncementID`,`HeadName`,`sadmissionno`,`sname`,`sclass`,`srollno`,`Amount`,`PaymentMode`,`TxnId`,`TxnStatus`,`PGTxnId`,`FeeReceipt`,`HeadType`,'$Year' from `fees_misc_collection_tmp` where `TxnId`='$txnid'") or die(mysql_error());
			}
			
			 $rsHead=mysql_query("SELECT `HeadName` FROM  `fees_misc_collection` where `TxnId`='$txnid'");
