...
$rsSummerUni=mysql_query("select distinct `category`, count(*) from (SELECT a.*,(select `Category` from `Commerce_Product` where `ProductId`=a.`Product_Id`) as `category` FROM `Commerce_OrderDetail` as `a`) as `x` group by `category`");
...
$rs=mysql_query("select count(*) from (SELECT a.*,(select `Category` from `Commerce_Product` where `ProductId`=a.`Product_Id`) as `category` FROM `Commerce_OrderDetail` as `a`) as `x`");
$rowTotalSale=mysql_fetch_row($rs);
$TotalSales=$rowTotalSale[0];
...
<body>
		...
		<div class="row-fluid">
			...
			<div class="row-fluid hideInIE8 circleStats">
				...
				<?php
				while($row=mysql_fetch_row($rsSummerUni))
				{
					$Category=$row[0];
					$CatTotalSale=$row[1];
					$arrnum=rand(1,5);
					$DivColor=$arrCol[$arrnum];
					$PercentSale=($CatTotalSale/$TotalSales)*100;
				?>
				<div class="span2" onTablet="span4" onDesktop="span2">
                	<div class="circleStatsItemBox <?php echo $DivColor;?>">
						<div class="header"><?php echo $Category;?> Sales</div>
						<span class="percent">Nos</span>
						<div class="circleStat">
                    		<input type="text" value="<?php echo $PercentSale;?>" class="whiteCircle" />
						</div>		
						<div class="footer">
							<span class="count">
								<span class="number"><?php echo $CatTotalSale;?></span>
