...
if($_POST["isSubmit"]=="yes")
{
	...
	$rsCheck=mysql_query("select * from `Assignment_Topic` where UCASE(`TopicName`)=UCASE('$txtTopic')");
	if(mysql_num_rows($rsCheck)>0)
	{
		$Msg="TopicName:".$txtTopic." already Exists!";
	}
	else
	{
	$rs=mysql_query("insert into `Assignment_Topic` (`sclass`,`Subject`,`ChapterName`,`TopicName`) values ('$txtclass','$txtSubject','$txtChapter','$txtTopic')");
		echo $rs;
...
<form name="frmDocument" id="frmDocument" method="post" action="CreateTopic.php">
...
<input type="hidden" name="isSubmit" id="isSubmit" value="yes">
	...
	<tr>
		...
		<td style="border-style: none; border-width: medium">
							   ...
							   <select name="txtChapter" id="txtChapter" style="float: left" class="text-box"	>
	...
	<tr>
		...
		<font face="Cambria">Enter Topic Name</font></b></td>

		<td colspan="2" style="border-style: none; border-width: medium">

		

			<input name="txtTopic" id="txtTopic" type="text" class="text-box"  required="true"></td>
...
<table border="1" width="100%" style="border-collapse: collapse" class="CSSTableGenerator">
	...
	$rs=mysql_query("select `sclass`,`Subject`,`ChapterName`,`TopicName` from `Assignment_Topic`");
	...
	<tr>
		...
		<td align="center" class="style2" width="321"><?php echo $Chapter;?></td>
