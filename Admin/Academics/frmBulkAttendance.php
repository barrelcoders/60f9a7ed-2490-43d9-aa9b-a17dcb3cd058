...
if ($_REQUEST["isSubmit"]=="yes")
{
	...
	for ($i=1;$i<=$TotalClass;$i++)
	{
		...
		if (mysql_num_rows($resultchk) == 0)
		{
			...
			elseif ($strAbsentee != "")
			{
				...
				$rsStudentDetail=mysql_query("select `sname`,`srollno`,`sclass`,`smobile` from `student_master` where `sclass`='$Class'");
				$StudetnMobile="";
				while($row = mysql_fetch_assoc($rsStudentDetail))
   				{
   					$StudentName=$row['sname'];  
						...
						//echo "String Found";//ABSENTEES ROLL NO IS FOUND WITH STUDENT ROLL NO
							...
							$ssql="INSERT INTO `attendance` (`sname`,`srollno`,`sclass`,`attendancedate`,`attendance`) VALUES ('$StudentName','$StudentRollNo','$StudentClasss','$AttandanceDt','A')";
		   					...
		   					//sending SMS to Absentee only for class 9****
		   						$pos= strpos($StudentClasss,"9");
		   						$pos1= strpos($StudentClasss,"10");
		   						$pos2= strpos($StudentClasss,"11");
		   						$pos3= strpos($StudentClasss,"12");
		   						$pos= strpos($StudentClasss,"Test");
			   					//if ($pos !== false)
			   					if ($pos !== false || $pos1 !== false || $pos2 !== false || $pos3 !== false)
								{
									...
									$url="http://messagebhejo.com/submitsms.jsp?user=Eduworld&key=ea0e1f127cXX&mobile=".$mobileno."&message=".$message."&senderid=DPSFBD&accusage=1";
	...
	$Msg="Bulk attandance submitted successfully!";
...
function removeAllOptions(selectbox)
	...
	function fnlCheckName(cnt)
	{
		ctrlClass="txtClass"+cnt;
		//alert(document.getElementById(ctrlClass).value);
		
		ctrlAbsentees="txtAbsentees"+cnt;
		//ctrlAbsenteesName="txtAbsenteesName" + cnt;
		ctrlAbsenteesName="tdAbsenteeName"+cnt;
		//alert(document.getElementById(ctrlAbsentees).value);
			      ...
			      if(xmlHttp.readyState==4)
			        {
			        	...
			        	*/
						//alert(rows);														
						document.getElementById(ctrlAbsenteesName).innerHTML =rows;
			        }
		      }
			var submiturl="GetAbsenteeName.php?Absentees=" + document.getElementById(ctrlAbsentees).value + "&Class=" + document.getElementById(ctrlClass).value;
			xmlHttp.open("GET", submiturl,true);
			xmlHttp.send(null);
...
<p class="style9"><strong><?php echo $Msg; ?></strong></p>
	...
	<tr>
		...
		<table width="100%" cellpadding="0" class="style6">
			...
			<tr>
				...
				<td style="width: 299px; height: 36px;" class="style7"><strong>Class</strong></td>

				<td style="width: 299px; height: 36px;" class="style8">
				...
				<td style="width: 300px; height: 36px;" class="auto-style21">
			...
			<tr>
				...
				<td style="width: 299px; height: 21px;" class="style7">
				...
				<td style="width: 299px; height: 21px;" class="style8">
				<input name="btnCheckName<?php echo $Cnt;?>" id="btnCheckName<?php echo $Cnt;?>" type="button" value="Check Name" onclick="Javascript:fnlCheckName(<?php echo $Cnt;?>)"></td>
				<td style="width: 300px; height: 21px;" id="tdAbsenteeName<?php echo $Cnt;?>">
				</td>
			</tr>
			<?php				
			}
