...
$result="select `EmpId`,`Name`,`Department`,`Designation`,(SELECT distinct `PrincipalRemark` from `Employee_ACR_HODAssesment` where `EmpId`=a.`EmpId`) from `employee_master` as `a` where 1 ";
	...
	$result=$result." order by CAST(replace(`EmpId`,'DPS','') AS UNSIGNED)";
...
<form name="frmRpt" method="post" action="EmployeeAssesmentPrincipal.php">
...
<tr>
	...
	<td align="center"><font face="Cambria">
		...
		<a href='EmployeeSelfAppraisal2.php?txtEmpId=<?php echo $EmpId;?>' target='_blank'>
		<strong>

		<input name="BtnSubmit" type=button value="Proceed" class="text-box" style="font-weight: 700"></strong></a></font></td>
	

	<td align="center"><font face="Cambria"><?php 
	if($Remarks!='')
	{
	echo "Assesment Completed";
	}
	else
	{
	
	echo "Assesment In-Complete";
		}
