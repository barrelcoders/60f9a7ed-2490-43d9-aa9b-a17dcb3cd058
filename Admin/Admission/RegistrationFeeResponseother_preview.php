...
?>
 <?php
  $Applicationno=$_REQUEST['ApplicationNo'];
   $sqlReport="SELECT `registration_no`, `sclass`, `TxnAmount`, `TxnStatus`, `pgTxnNo`, `TxRefNo`, `TxMsg` FROM `student_registration_others` WHERE registration_no =  ".$Applicationno."";
  //echo $sqlReport;die;
...
.text2{font-size:14px;} .text3{font-size:14px;} .xs{font-size:10px;} }	
  ...
  <?php if($reportData['TxnStatus']=="SUCCESS") { ?>
       <div class="text1"><i><b>Class <?php echo $reportData['sclass']; ?></b> admission.</i><br> Your Application Number is <b><?php echo $reportData['registration_no']; ?>.</b></div>
    ...
    <div class="head">Sorry !</div>
       <div class="text1"><i>Your Payment Process is not completed for <b>Class <?php echo $reportData['sclass']; ?></b> admission.</i><br /> Please try Again.</div>
       
        <div class="text3">For further updates visit school website regularly.  </div>
         <?php } ?>
    <div>
	<div class="col-xs-6" align="center" id="preview"><a href="../../Admission/student_registration_others_preview.php?ApplicationNo=<?php echo $reportData['registration_no']; ?>" target="_blank">PREVIEW</a></div>
