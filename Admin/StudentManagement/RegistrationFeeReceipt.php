...
if ($_REQUEST["txtName"] != "")
{
	...
	//if ($Class=="PRE-NURSERY" || $Class=="LKG" || $Class=="UKG")
	...
	while($row = mysql_fetch_row($rsPreFix))
	{
		...
		$ssqlRegi="SELECT * FROM `NewStudentRegistration` where `RegistrationNo` like '$PreFix"."%'";	
		$rsRegiNo= mysql_query($ssqlRegi);
			
		if (mysql_num_rows($rsRegiNo) > 0)
		{
			
			$ssqlRegi="SELECT max(CAST(replace(a.`RegistrationNo`,'".$PreFix."','') AS SIGNED INTEGER)) FROM `NewStudentRegistration` as `a` where `RegistrationNo` like '".$PreFix."%'";	
		...
		{
			//$NewRistrationNo=$PreFix."1";
			$NewRistrationNo="1";
		}
//$Admission=$_REQUEST["txtAdmission"];
	
		$Admission=$PreFix.$NewRistrationNo;
...
$Dt=$_REQUEST["txtDate"];
$arr=explode('/',$Dt);
		$RegistrationDate= $arr[2] . "-" . $arr[0] . "-" . $arr[1];
