...
<?php
		...
		//commented on 8-Mar-2015
			...
			$SchoolId=$_REQUEST["txtSchoolId"];
			
			$rsPreFix=mysql_query("select `PREFIX` from `SchoolConfig` where `SchoolId`='$SchoolId'");
			...
			$ssqlRegi="SELECT max(CAST(replace(a.`RegistrationNo`,'".$PreFix."','') AS SIGNED INTEGER)) FROM (SELECT distinct `RegistrationNo` from `NewStudentRegistration` where `RegistrationNo` like '".$PreFix."%' union SELECT distinct `sadmission` FROM `Almuni` where `sadmission` like '".$PreFix."%' union SELECT distinct `sadmission` FROM `student_master` where `sadmission` like '".$PreFix."%') as `a`";	
			$rsRegiNo1= mysql_query($ssqlRegi);
			while($row2 = mysql_fetch_row($rsRegiNo1))
			{
	
						$NewRistrationNo1=$row2[0]+1;
	
			}
		//$Admission=$_REQUEST["txtRegNo"];
		$Admission=$NewRistrationNo1;
		
		$ssql ="select * FROM `AdmissionFees` where  `sadmission`='$Admission'";
		$rChk = mysql_query($ssql,$Con);
		if (mysql_num_rows($rChk) > 0)
		{
			echo "<br><br><center><b>Admission fee for :".$Admission." already submited !<br>Please click <a href='StudentManagementLandingPage.php'>here</a> to go home";
			exit();
		}
		...
		if (mysql_num_rows($result)==0)
		{
			...
			$ssql="INSERT INTO `NewStudentAdmission` (`RegistrationNo`,`sname` , `DOB`, `Sex`, `Category`, `Nationality`,`sadmission`,`AdmissionDate`,`sclass`,`MasterClass`,`srollno`,`LastSchool`, `sfathername`,`FatherEducation`, `FatherOccupation`, `MotherName`, `MotherEducation`,`smobile`,`AlternateMobile`,`simei`,`suser`,`spassword`,`email`,`routeno`,`ProfilePhoto`,`FinancialYear`,`PlaceOfBirth`,`Age`,`MotherTongue`,`ResidentialAddress`,`PermanentAddress`,`sfatherage`,`FatherDesignation`,`FatherAnnualIncome`,`FatherCompanyName`,`FatherOfficeAddress`,`FatherOfficePhoneNo`,`MotherAge`,`MotherOccupatoin`,`MotherDesignation`,`MontherAnnualIncome`,`MotherCompanyName`,`MotherOfficeAddress`,`MotherOfficePhone`,`GeneralInformation`,`ContributionArea`,`StudentWeeknessStrength`,`SpecialAttentionDetail`,`StudetnPreviousHistory`,`RealBroSisName`,`RealBroSisSchool`,`RealBroSisClass`,`DiscontType`,`SchoolId`) VALUES('$RegNo','$Name','$DOB','$Sex','$Category','$Nationality','$Admission','$AdmissionDate','$Class','$MasterClass','$RollNo','$LastSchool','$Father','$FatherEducation','$FatherOccupation','$MotherName','$MotherEducation','$MobileNo','$AlternatMobileNo','$Imei','$UserId','$Password','$email','$Route','$ProfilePic','$FYear','$PlaceOfBirth','$Age','$MotherTongue','$ResidentialAddress','$PermanentAddress','$sfatherage','$FatherDesignation','$FatherAnnualIncome','$FatherCompanyName','$FatherOfficeAddress','$FatherOfficePhoneNo','$MotherAge','$MotherOccupatoin','$MotherDesignation','$MontherAnnualIncome','$MotherCompanyName','$MotherOfficeAddress','$MotherOfficePhone','$GeneralInformation','$ContributionArea','$StudentWeeknessStrength','$SpecialAttentionDetail','$StudetnPreviousHistory','$RealBroSisName','$RealBroSisSchool','$RealBroSisClass','$DiscountType1','$SchoolId')";
			...
			//mysql_query($ssql1) or die(mysql_error());
			$ssql="INSERT INTO `AdmissionFees` (`sadmission`,`sname`,`sclass`,`amountpaid`,`quarter`,`FinancialYear`,`receipt`,`EntryDate`,`FeeReceiptCode`) values ('$Admission','$sname','$sclass','$GrandTotal','Q1','2015','$ChallanNo','$currentdate','$sstr')";
			mysql_query($ssql) or die(mysql_error());
...
<div id="MasterDiv" style="border-style: solid; border-width: 1px; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px" >
	...
	<p align="center" ><b><font face="Cambria">Admission Fees Receipt</font></b></p>
	
	<p align="center"><b>Receipt No.<?php echo $NewReciptNo; ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
	&nbsp;Date:<?php echo date("d-m-Y"); ?></b></p>
	...
	<div align="center">
		...
		<table style="width: 66%" class="style1">
			<tr>
				<td>
				<ul>
					<li><font face="Cambria"><em>Admission Fees &amp; Annual Charges 
					once paid will not be refunded</em></font> </li>
				</ul>
				</td>
			</tr>
		</table>
...
<div id="divPrint">
	...
	<p align="center">
	<font face="Cambria">
	<a href="Javascript:printDiv();">PRINT</a> || <a href="StudentManagementLandingPage.php">
	HOME</a>	
</font> 
