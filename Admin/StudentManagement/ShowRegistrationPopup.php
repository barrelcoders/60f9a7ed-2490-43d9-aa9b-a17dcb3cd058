...
if($RegNo !="")
{
		$rs=mysql_query("select `RegistrationNo`,CONCAT(`sname`,' ',`slastname`) as `student_name`,`sfathername`,`MotherName`,`ProfilePhoto`,`smobile`,`sclass` from `NewStudentRegistration` where `RegistrationNo`='$RegNo' and `status`='Selected'");
...
if($_REQUEST["isSubmit"]=="yes")
{
	...
	////SENDING SMS........
					...
					//Sending SMS
					//$notice=%20
					$notice="Congratulations!!!  Your ward has been selected for admission to class  ". $class . "  for the academic session 2017-18. The admission is subject to verification of documents.    Principal <br>DPS Faridabad ";
					...
					//$url="http://messagebhejo.com/submitsms.jsp?user=Eduworld&key=ea0e1f127cXX&mobile=" . $smobile. "&sms=" . $notice . "&senderid=INFOSM";
