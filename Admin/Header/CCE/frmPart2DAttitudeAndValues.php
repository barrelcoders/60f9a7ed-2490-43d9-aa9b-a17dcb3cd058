...
<?php
	...
	$ssqlChk="select * from `reportcard_interim` where `sadmission`='$AdmissionId' and `indicatortype`='Attitude Values'";
	$rsChk = mysql_query($ssqlChk);
	if (mysql_num_rows($rsChk) > 0)
	{
		$Msg="<br><br><center>Step 3 (Co-Scholastic- Attitude) has been uploaded!";
	}
	
	$sqlRcpt = "SELECT `DescriptiveIndicator` FROM  `reportcard_attitudevalues` where `status`='1' and `indicatortype`='Attitude Values' and `class`='$sclass'";
...
<script language="javascript">
	function fnlGetGrade(cnt)
	{
		...
		try
			...
			var submiturl="GetGradePart2LifeSkill.php?Point=" + escape(document.getElementById(ctrlname).value) + "&indicater_type=AttitudeValues&class=" + document.getElementById("txtClass").value;
...
<table style="width: 100%" class="style1">
	<tr>
		...
		<span class="style11">Part - 2 (B) Co-Scholastic Areas Attitude &amp; Values</span><span class="style7">
	...
	<input type="hidden" name="txtIndicaterType" id="txtIndicaterType" value="Attitude Values">
	</span>
	<tr>
		<td class="style8" style="width: 55px; height: 15px"><strong>S. No.</strong></td>
		<td class="style3" style="height: 15px; width: 518px;"><strong>Descriptive Indicators</strong></td>
		<td class="style3" style="height: 15px; width: 176px;"><strong>Point</strong></td>
		<td class="style3" style="height: 15px; width: 177px;"><strong>Grade</strong></td>
		<td class="style3" style="height: 15px; width: 177px;"><strong>Description</strong></td>
	</tr>
	<?php
		$ccnt=1;
