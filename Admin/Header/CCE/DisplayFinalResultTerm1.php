...
<body>
<span style="font-weight: 700"><font face="Cambria">Term1 Marks Details</font></span><hr  style="height: -15px">
		<p>
		<font face="Cambria">
		<a href="javascript:history.back(1)">
<img height="30" src="unsaved:///../images/BackButton.png" width="70" style="float: right" ></a></font></p>
<table border="1" style="border-collapse: collapse" cellspacing="0" cellpadding="0" width="100%">
<tr>
	...
	for($i=1;$i < count($arrSubject);$i++)
		{
			$Subject = str_replace(" ","",$arrSubject[$i]);
?>
<th colspan="3" bgcolor="#95C23D"><font face="Cambria">FA1</font></th>
<th colspan="3" bgcolor="#95C23D"><font face="Cambria">FA2</font></th>
<th colspan="3" bgcolor="#95C23D"><font face="Cambria">SA1</font></th>
<th colspan="3" bgcolor="#95C23D"><font face="Cambria">Term1</font></th>
<th colspan="3" bgcolor="#95C23D"><font face="Cambria">FA3</font></th>
<th colspan="3" bgcolor="#95C23D"><font face="Cambria">FA4</font></th>
<th colspan="3" bgcolor="#95C23D"><font face="Cambria">SA2</font></th>
<th colspan="3" bgcolor="#95C23D"><font face="Cambria">Term2</font></th>
<?php } ?>
</tr>
...
while($row = mysql_fetch_array($rs1))
  {
  	...
  	$StudentClass=$row[1];
  	$StudentRollNo=$row[2];
  	$rsFName= mysql_query("select `sfathername` from `student_master` where `sclass`='$StudentClass' and `srollno`='$StudentRollNo'");
		...
		for($i=1;$i < count($arrSubject);$i++)
			{
				...
				$rsFA1= mysql_query("select `MarksObtained`,`convertedmarks`,`Grade`,`MaxMarks` from `report_card_temp` where `sclass`='$Class' and `srollno`='$StudentRollNo' and `subject`='$Subject' and `testtype`='FA1'");
				$rsFA2= mysql_query("select `MarksObtained`,`convertedmarks`,`Grade`,`MaxMarks` from `report_card_temp` where `sclass`='$Class' and `srollno`='$StudentRollNo' and `subject`='$Subject' and `testtype`='FA2'");
				$rsFA3= mysql_query("select `MarksObtained`,`convertedmarks`,`Grade`,`MaxMarks` from `report_card_temp` where `sclass`='$Class' and `srollno`='$StudentRollNo' and `subject`='$Subject' and `testtype`='FA3'");
				$rsFA4= mysql_query("select `MarksObtained`,`convertedmarks`,`Grade`,`MaxMarks` from `report_card_temp` where `sclass`='$Class' and `srollno`='$StudentRollNo' and `subject`='$Subject' and `testtype`='FA4'");
				$rsSA1= mysql_query("select `MarksObtained`,`convertedmarks`,`Grade`,`MaxMarks` from `report_card_temp` where `sclass`='$Class' and `srollno`='$StudentRollNo' and `subject`='$Subject' and `testtype`='SA1'");
				$rsSA2= mysql_query("select `MarksObtained`,`convertedmarks`,`Grade`,`MaxMarks` from `report_card_temp` where `sclass`='$Class' and `srollno`='$StudentRollNo' and `subject`='$Subject' and `testtype`='SA2'");
...
<td>
	...
	$ssql="INSERT INTO `report_card_rank` (`sclass`,`srollno`,`sname`,`TotalMaxMarks`,`TotalObtainedMarks`,`PercentObtained`) VALUES ('1A','$StudentRollNo','$StudentName','$SumOfMaxMarks','$SumOfObtainedMarks','$PercentObtained')";
