...
$ssql1="select `srno`,`RegistrationNo`,CONCAT(`sname`,' ',`slastname`) as `sname`,`sclass`,`TxnAmount`,`RegistrationFormNo`,DATE_FORMAT(`RegistrationDate`,'%d-%m-%Y') as `date`,'Paid',`TxnId`,(select `PGTxnId` from `NewStudentRegistration_temp` where `TxnId`=a.`TxnId` limit 1) as `PGTxnId`,`TxnStatus`,`PaymentMode`,DATE_FORMAT(`ChequeDate`,'%d-%m-%Y') as `ChequeDate`,`ChequeBounceAmt` from  `NewStudentRegistration_2016` as `a` where 1=1";
...
if($DateFrom!="")
{
	...
	$ssql1=$ssql1." and `RegistrationDate`>='$DateFrom' and `RegistrationDate`<='$DateTo' order by `RegistrationFormNo` limit 2000";

}

$rsRegistration=mysql_query($ssql);
$rsRegistration1=mysql_query($ssql1);
 ...
 <table id="table3" class="CSSTableGenerator" cellspacing="0" cellpadding="0" style="width: 100%" >
	...
	?></strong></span></span><strong><font style="font-size: 12pt"> </font> </strong></td>
   	...
   	<?php
   		...
   		while($rowR1 = mysql_fetch_row($rsRegistration1))
   		{
   			$srno=$rowR1[0];
   			$RegistrationNo=$rowR1[1];
   			$sname=$rowR1[2];
   			$sclass=$rowR1[3];
   			$TxnAmount=$rowR1[4];
   			$RegistrationFormNo=$rowR1[5];
   			$RegistrationDate=$rowR1[6];
   			$State=$rowR1[7];
   			$TxnId=$rowR1[8];
   			$PGTxnId=$rowR1[9];
   			$PaymentMode=$rowR1[11];
   			 $TxnStatus=$rowR1[10];
   			 $ChequeDate=$rowR1[12];
   			 $cheque_bounce_amt=$rowR1[13];
