...
$ssqlMisc="select `srno`,`sadmissionno`,`sname`,`sclass`,`Amount`,`FeeReceipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date`,'Paid',`TxnId`,`PGTxnId`,`PaymentMode`,`ChequeNo`,`Chequedate`,`HeadName`,`VendorName`,`ChequeBounceAmt` from `fees_misc_collection` as `a` where `HeadType` ='Hostel'";
$ssqlReg="select `srno`,`sadmission`,`sname`,`sclass`,`amountpaid`,`receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date`,'Paid',`TxnId`,`PGTxnId`,`PaymentMode`,`AdjustedLateFee`,`cheque_bounce_amt` from  `fees_hostel` as `a` where `receipt` like 'HR%' and  1=1";
   	...
   	<tr>
		   ...
		   <td   align="center" class="style2" style="width: 110px; height: 35px;"   >
            ...
            <?php echo $cheque_bounce_amt; ?>			</td>
   		   ...
   		   <td   align="center" class="style2" style="width: 110px; height: 35px;"  >
			<font face="Cambria" size="3"><a href='<?php echo $ShowReceiptFileName;?>?srno=<?php echo $srno; ?>' target="_blank"><?php echo $RegistrationFormNo;?></a> </font></td>
