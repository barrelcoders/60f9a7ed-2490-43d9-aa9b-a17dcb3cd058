...
?>
<head>
<title>Daily Classwork Not Given Report</title>
...
if ($_REQUEST["isSubmit"]=="yes")
{
	...
	//$ssql="select x.`class`,x.`subject`,x.`TodayClassWork` from (SELECT distinct `class`,`subject`,(SELECT `classwork` FROM `classwork_master` WHERE `sclass`=a.`class` and `subject`=a.`subject` and `classworkdate`='$currentdate1') as `TodayClassWork` FROM `subject_master` as `a` WHERE status='1') as `x` where x.`TodayClassWork`='' or x.`TodayClassWork` is NULL order by x.`class`";
	$ssql="select x.`class`,x.`subject`,x.`TodayClassWork` from (SELECT distinct `class`,`subject`,(SELECT `classwork` FROM `classwork_master` WHERE `sclass`=a.`class` and `subject`=a.`subject` and `classworkdate`='$StartDt') as `TodayClassWork` FROM `subject_master` as `a` WHERE status='1') as `x` where x.`TodayClassWork`='' or x.`TodayClassWork` is NULL order by x.`class`";
	$rs= mysql_query($ssql);
}
...
<p>&nbsp;</p>
<p><b><font face="Cambria">Daily Classwork Not Given Report</font></b></p>
<hr>
<p>&nbsp;</p>
<table style="width: 100%" class="style1">
<form name="frmReport" id="frmReport" method="post" action="DailyClassWorkNotGivenReport.php">
<input type="hidden" name="isSubmit" id="isSubmit" value="yes">
	<tr>
		<td class="style4" style="width: 144px">Start Date:</td>
		<td class="style2" style="width: 148px"><input name="txtDate1" id="txtDate1" class="tcal" style="width: 108px" type="text"></td>
		<td class="style4">
...
if ($_REQUEST["isSubmit"]=="yes")
{
	...
	$strTable=$strTable.'<td colspan="4" align="center"><b><font face="Calibri">Daily Classwork Report ('.$ShowDate.')</font></b></td>';
	$strTable=$strTable.'</tr>';
	$strTable=$strTable.'<tr>';
