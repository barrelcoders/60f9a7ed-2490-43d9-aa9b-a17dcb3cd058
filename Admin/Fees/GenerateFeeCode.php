...
$admissionno= $_POST["admissionno"];
	...
	$ssql="SELECT  `srno` ,  `sadmission` ,  `sname` ,  `sclass` ,  `srollno` , `quarter`, `fees_amount` ,  `amountpaid` ,  `BalanceAmt` ,  `FinancialYear` ,  `receipt` ,  `date` ,  `refundamount` , `refunddate` ,  `cancelamount` ,  `canceldate` ,  `finalamount` ,  `chequeno` ,  `cheque_date` ,  `bankname` ,  `ActualLateFee` ,  `ActualDelayDays` ,  `AdjustedLateFee` , `AdjustedDelayDays` ,  `Remarks` , (SELECT  `ReceiptCode` FROM  `fees_receipt_code` WHERE  `sadmission` = a.`sadmission`) AS  `ReceiptCode` FROM  `fees` AS  `a` WHERE  `sadmission`='$admissionno'";
		$rs= mysql_query($ssql);
}
...
<table style="width: 100%" class="style2">
	<form name="frmTransaction" id="frmTransaction" method="post" action="GenerateFeeCode.php">
	<input type="hidden" name="isSubmit" id="isSubmit" value="yes">
	<tr>
		<td style="width: 152px" class="style4"><strong>Enter Admission No:</strong></td>
		<td style="width: 154px" class="style3">
		<input name="admissionno" id="admissionno" class="text-box" type="text"></td>
...
<table border="0" cellpadding="0" class="CSSTableGenerator">
	...
	<tr>
		...
		<a href="DummyFeeReceipt.php"><input name="PrintQ1Receipt" style="height: 29px; width: 99px;" type="button" value="Print Reciept" class="auto-style1"></a>
		</td>		
	</tr>
	</table>
<?php
