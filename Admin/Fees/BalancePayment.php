...
$rs= mysql_query($ssql);
	...
	if (mysql_num_rows($rs) > 0)
	{
		...
		{
			//Quarterly
			//$ssql="select x.`sadmission`,x.`sname`,x.`sclass`,x.`BalanceAmt`,x.`receipt` from (select * from `fees` order by `FinancialYear` desc) `x` where `sadmission`='$sadmission' order by `quarter` ,CAST(`FinancialYear` AS UNSIGNED) desc";
			$ssql="select `sadmission`,`sname`,`sclass`,`BalanceAmt`,`receipt` FROM `fees` WHERE `sadmission` = '$sadmission' AND CAST(`FinancialYear` AS UNSIGNED)=(select MAX(CAST(`FinancialYear` AS UNSIGNED)) from `fees` AS `x` where `sadmission`='$sadmission') ORDER BY CAST( REPLACE(`quarter`,'Q','') AS UNSIGNED) DESC";
		...
		else
		{
			...
			//CALCULATING CURRENT BALANCE
				$ssql="select x.`sadmission`,x.`sname`,x.`sclass`,x.`BalanceAmt`,x.`receipt` from (select * from `fees_monthwise` order by `FinancialYear` desc) `x` where `sadmission`='$sadmission' order by STR_TO_DATE(`FeeMonth`,'%b') desc";
				$rs1= mysql_query($ssql);
				if (mysql_num_rows($rs1) > 0)
					{
						while($row = mysql_fetch_row($rs1))
...
<table cellpadding="0" style="padding: 1px 4px; width: 677px; " class="style1" cellspacing="3" >
			...
			<tr>
				...
				<select name="cboPaymentMode" id="cboPaymentMode"  class="text-box" onchange="Javascript:CheckPaymentMode();">
			...
			<tr>
				...
				<input name="txtChequeNo" id="txtChequeNo" type="text" class="text-box" readonly="readonly"></td>
			...
			<tr>
				...
				<input name="txtBankName" id="txtBankName" class="text-box" type="text" readonly="readonly"></td>
			...
			<tr>
				...
				<input name="txtName" id="txtName" value="<?php echo $Name; ?>" class="text-box" readonly="readonly" type="text"></td>
			...
			<tr>
				...
				</font><input name="txtAdmissionNo" id="txtAdmissionNo" value="<?php echo $AdmissionId; ?>" readonly="readonly" class="text-box" type="text"></td>
			...
			<tr>
				...
				</font><input name="txtClass" id="txtClass" value="<?php echo $Class; ?>" type="text" class="text-box" readonly="readonly"></td>
			...
			<tr>
				...
				</font><input name="txtBalancePayable" id="txtBalancePayable" value="<?php echo $CurrentBalance; ?>"class="text-box" type="text" readonly="readonly"></td>
			...
			<tr>
				...
				<input name="txtPaidBalance" id="txtPaidBalance" class="text-box" type="text" onkeyup="Javascript:CalculateCurrentBalance();"></td>
			...
			<tr>
				...
				<input name="txtBalanceForward" id="txtBalanceForward" class="text-box" type="text"></td>
