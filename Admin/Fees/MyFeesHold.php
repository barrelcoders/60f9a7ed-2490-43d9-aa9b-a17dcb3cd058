...
$sql = "SELECT `FinancialYear`,`Hostel`,`DiscontType` FROM `student_master` as `a` where `sadmission`='$sadmission'"; 
$rsSt = mysql_query($sql, $Con);
while($rowSt=mysql_fetch_row($rsSt))
{
$FinancialYear=$rowSt[0];
$StudentHostel=$rowSt[1];
$DiscountType=$rowSt[2];
break;
}
	
	//echo $FinancialYear."/".$CurrentFinancialYear;
	//exit();
	...
	if($FinancialYear<$CurrentFinancialYear)
	{
		...
		$rsYearlyConcessionAmt=mysql_query("SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `FeesType` !='Hostel' and `financialyear`='$CurrentFinancialYear' and (`feeshead`='Total Concession Amount' or `feeshead`='TOTAL CONCESSION AMOUNT')");
		...
		$rsCheckQ1=mysql_query("SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `feeshead`='Fees First Installment'");
			...
			if (mysql_num_rows($rsCheckQ1) == 0)
			{
				...
				else
				{
					...
					{
						//echo "SELECT `amount` FROM `fees_master` WHERE `quarter`='Q1' and `financialyear`='$CurrentFinancialYear' and `feeshead`='TUITION FEES' and `StudentType`='old'";
						//exit();
						$rsFeePaidQ1=mysql_query("SELECT `amount` FROM `fees_master` WHERE `quarter`='Q1' and `financialyear`='$CurrentFinancialYear' and `feeshead`='TUITION FEES' and `StudentType`='old'");
						while($rowFPQ1=mysql_fetch_row($rsFeePaidQ1))
						{
							if($DiscountType=="EWS" || $DiscountType=="Staff Child")
							$FeeAmtPaidQ1=0;
							else
			...
			else
			{
				...
				{				
					$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date` from `fees` where `sadmission`='$sadmission' and `quarter`='Q1' and `FinancialYear`='$CurrentFinancialYear' and `FeesType` !='Hostel'");
		...
		//CHECK FEE SUBMISSION IN FEE TABLE FOR QUARTER Q2 IF FOUND THEN SHOW RECEIPT DETAIL OTHER WISE TO PAID AMOUNT WILL BE SHOWN//////////////////
			...
			else
			{
				...
				while($rowFPQ2=mysql_fetch_row($rsCheckQ2))
				{
					...
					$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date`  from `fees` where `sadmission`='$sadmission' and `quarter`='Q2' and `FinancialYear`='$CurrentFinancialYear' and `FeesType` !='Hostel'");
					while($rowRcpt=mysql_fetch_row($rsRcptNo))
					{
		...
		//CHECK FEE SUBMISSION IN FEE TABLE FOR QUARTER Q3 IF FOUND THEN SHOW RECEIPT DETAIL OTHER WISE TO PAID AMOUNT WILL BE SHOWN//////////////////
			...
			else
			{
				...
				{
					$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date`,`AdjustedLateFee` from `fees` where `sadmission`='$sadmission' and `quarter`='Q3' and `FinancialYear`='$CurrentFinancialYear' and `FeesType` !='Hostel'");
					while($rowRcpt=mysql_fetch_row($rsRcptNo))
					{
		...
		//CHECK FEE SUBMISSION IN FEE TABLE FOR QUAERTER Q4
			$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date` from `fees` where `sadmission`='$sadmission' and `quarter`='Q4' and `FinancialYear`='$CurrentFinancialYear' and `FeesType` !='Hostel'");
					while($rowRcpt=mysql_fetch_row($rsRcptNo))
					{
			...
			$rsCheckQ4=mysql_query("SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `feeshead`='FEES FOURTH INSTALLMENT'");
			if (mysql_num_rows($rsCheckQ4) > 0)
			{
				while($rowRcpt=mysql_fetch_row($rsCheckQ4))
				{
		...
		///HOSTEL AMOUNT CALCULATION**************************
							...
							$rsYrTotalHostelAmt=mysql_query("SELECT sum(`amount`) FROM `fees_student` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `FeesType`='Hostel'");
							while($rowYr=mysql_fetch_row($rsYrTotalHostelAmt))
							{
								$ToalHostelAmtPayableYearly=$rowYr[0];
								break;
							}
							
							$rsHostelAdvance=mysql_query("select `amount` from `fees_student` where `feeshead`='HOSTEL AMOUNT CREDIT' and `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear'");
							while($rowH = mysql_fetch_row($rsHostelAdvance))
							{
						...
						$rsHostelPaidAmt=mysql_query("select sum(`amountpaid`) from `fees` where `sadmission`='$sadmission' and `FinancialYear`='$FeeSubmissionFinancialYear' and `FeesType` ='Hostel' and cheque_status !='Bounce'");
									while($row = mysql_fetch_row($rsHostelPaidAmt))
									{
							    ...
							    $rsCheckQ1=mysql_query("SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `feeshead`='HOSTEL FIRST INSTALLMENT'");
								if (mysql_num_rows($rsCheckQ1) == 0)
								{	
									if($StudentType=="NewStudent")
									{
									...
									else
									{
										...
										$rsFeePaidQ1=mysql_query("SELECT `amount` FROM `fees_master` WHERE `quarter`='Q1' and `financialyear`='$CurrentFinancialYear' and `feeshead`='hostelfees' and `StudentType`='old'");
										while($rowFPQ1=mysql_fetch_row($rsFeePaidQ1))
										{
								...
								else
								{
									while($rowFPQ1=mysql_fetch_row($rsCheckQ1))
									{
										$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date` from `fees` where `sadmission`='$sadmission' and `quarter`='Q1' and `FinancialYear`='$CurrentFinancialYear' and `FeesType` ='Hostel' and cheque_status !='Bounce'");
										while($rowRcpt=mysql_fetch_row($rsRcptNo))
										{
							...
							//CHECK FEE SUBMISSION IN FEE TABLE FOR QUARTER Q2 IF FOUND THEN SHOW RECEIPT DETAIL OTHER WISE TO PAID AMOUNT WILL BE SHOWN//////////////////
								...
								$rsCheckQ2=mysql_query("SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `FinancialYear`='$CurrentFinancialYear' and `feeshead`='HOSTEL SECOND INSTALLMENT'");
								if (mysql_num_rows($rsCheckQ2) == 0)
								{
									$rsFeePaidQ2=mysql_query("SELECT `amount` FROM `fees_master` WHERE `quarter`='Q2' and `FinancialYear`='$CurrentFinancialYear' and `feeshead`='hostelfees'");
									while($rowFPQ2=mysql_fetch_row($rsFeePaidQ2))
									{
								...
								else
								{
									while($rowFPQ2=mysql_fetch_row($rsCheckQ2))
									{
										$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date`  from `fees` where `sadmission`='$sadmission' and `quarter`='Q2' and `FinancialYear`='$CurrentFinancialYear' and `FeesType` ='Hostel' and cheque_status !='Bounce'");
										while($rowRcpt=mysql_fetch_row($rsRcptNo))
										{
							...
							//exit();
								...
								$rsCheckQ3=mysql_query("SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `feeshead`='HOSTEL THIRD INSTALLMENT'");
								if (mysql_num_rows($rsCheckQ3) == 0)
								{
									$rsFeePaidQ3=mysql_query("SELECT `amount` FROM `fees_master` WHERE `quarter`='Q3' and `financialyear`='$CurrentFinancialYear' and `feeshead`='hostelfees'");
								...
								else
								{
									while($rowFPQ3=mysql_fetch_row($rsCheckQ3))
									{
										$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date` from `fees` where `sadmission`='$sadmission' and `quarter`='Q3' and `FinancialYear`='$CurrentFinancialYear' and `FeesType` ='Hostel' and cheque_status !='Bounce'");
										while($rowRcpt=mysql_fetch_row($rsRcptNo))
										{
							...
							//CHECK FEE SUBMISSION IN FEE TABLE FOR QUAERTER Q4
								...
								$rsCheckQ4=mysql_query("SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `feeshead`='HOSTEL FORTH INSTALLMENT'");
								if (mysql_num_rows($rsCheckQ4) > 0)
								{
									while($rowRcpt=mysql_fetch_row($rsCheckQ4))
									{
								...
								$rsHostelPaidAmt=mysql_query("select sum(`amountpaid`)-sum(`ActualLateFee`) from `fees` where `sadmission`='$sadmission' and `FinancialYear`='$CurrentFinancialYear' and `FeesType` ='Hostel' and cheque_status !='Bounce'");
								while($row = mysql_fetch_row($rsHostelPaidAmt))
								{
								...
								$ssql="select y.*,(`YearlyHostelPayable`-`HostelFeePaid`-`AdjustedAmtInHostelQ4`) as `HostelPayableInQ4` from
									...
									FROM `fees_student` as `a` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `feeshead`='TOTAL BILL AMOUNT'
									) as `x`
									) as `y`";
									$rsQ3RegularBalanceAmt=mysql_query($ssql);
									$rowR=mysql_fetch_row($rsQ3RegularBalanceAmt);
									if($rowR[9] !="")
									$HostelFeeAmtPaidQ4=$HostelFeeAmtPaidQ4-$rowR[9];
									
							$rsBalance=mysql_query("select SUM(`BalanceAmt`) from `fees` where `sadmission`='$sadmission' and `FinancialYear`='$CurrentFinancialYear' and `FeesType`='Hostel'");
							$rowH=mysql_fetch_row($rsBalance);
