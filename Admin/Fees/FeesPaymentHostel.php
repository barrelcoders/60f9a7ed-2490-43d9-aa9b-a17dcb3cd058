...
if ($_REQUEST["txtAdmissionNo"] != "")
{
	...
	while($rowM=mysql_fetch_row($rsMClass))
	{
		...
		$sqlQ1 = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime` FROM `fees` where  `sadmission`='$AdmissionNo' and `quarter`='Q1' and `FinancialYear`='$CurrentFinancialYear' and `FeesType`='Hostel' and cheque_status !='Bounce'";
		...
		$sqlQ2 = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime` FROM `fees` where  `sadmission`='$AdmissionNo' and `quarter`='Q2'  and `FinancialYear`='$CurrentFinancialYear' and `FeesType`='Hostel' and cheque_status !='Bounce'";
		...
		$sqlQ3 = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime` FROM `fees` where  `sadmission`='$AdmissionNo' and `quarter`='Q3'  and `FinancialYear`='$CurrentFinancialYear' and `FeesType`='Hostel' and cheque_status !='Bounce'";
		...
		$sqlQ4 = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime` FROM `fees` where  `sadmission`='$AdmissionNo' and `quarter`='Q4'  and `FinancialYear`='$CurrentFinancialYear' and `FeesType`='Hostel' and cheque_status !='Bounce'";
...
//-------------------
			...
			$sql = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime`,`FinancialYear`,`BalanceAmt` FROM `fees` where  `sadmission`='$AdmissionNo' and `FinancialYear`='$CurrentFinancialYear' and `FeesType`='Hostel' and cheque_status !='Bounce' ORDER BY `quarter` DESC";
			$rs1 = mysql_query($sql);
			
			//echo $sql."<br>".$StudentType;
			//exit();
		//If Student is Regular ant not paid in current financial year********
			...
			if (mysql_num_rows($rs1) == 0 && $StudentType == "Old Student")
			{
							...
							$sql = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime`,`FinancialYear`,`BalanceAmt` FROM `fees` where  `sadmission`='$AdmissionNo' and `FinancialYear`='$FeeSubmissionFinancialYear' and `FeesType`='Hostel' and cheque_status !='Bounce' ORDER BY `quarter` DESC";				
							$rs = mysql_query($sql);
							//echo $sql;
							//exit();
							if (mysql_num_rows($rs) > 0)
		...
		$rsAdvanceAmt=mysql_query("sum(`amount`) FROM `fees_student` where `sadmission`='$AdmissionNo' and `financialyear`='$FeeSubmissionFinancialYear'  and `feeshead`='Advances'");
		...
		//////HOSTEL CHARGE IS COMMENTED DUE TO IT IS SEPERATLY SUBMITTED
		 ...
		 if ($Hostel=="Yes")
		 {
			...
			if($FeeSubmissionQuarter =="Q1" || $FeeSubmissionQuarter =="Q2" || $FeeSubmissionQuarter =="Q3")
			{
				...
				$rsHostelPaidAmt=mysql_query("select sum(`amountpaid`) from `fees` where `sadmission`='$AdmissionNo' and `FinancialYear`='$FeeSubmissionFinancialYear' and `FeesType` ='Hostel'");
				...
				if(($YearlyTotalHostelAmt-$PaidHostelAmount)<$HostelCharge)
				{
			...
			if($FeeSubmissionQuarter =="Q4")
			{
				...
				$rsHostelPaidAmt=mysql_query("select sum(`amountpaid`)-sum(`ActualLateFee`) from `fees` where `sadmission`='$AdmissionNo' and `FinancialYear`='$FeeSubmissionFinancialYear' and `FeesType` ='Hostel' and cheque_status !='Bounce'");
				while($row = mysql_fetch_row($rsHostelPaidAmt))
				{
					$PaidHostelAmount=$row[0];
					break;
		...
		//echo $YearlyTotalHostelAmt."/".$PaidHostelAmount."/".$HostelCharge;
		//exit();
		/////////
...
//-------------------- Previous Payment history----------------------------------------------------------
	$ssql = "SELECT `quarter`,`fees_amount`,`amountpaid`,`BalanceAmt`,`status`,`receipt`,date_format(`date`,'%d-%m-%Y') as `date`,`FinancialYear`,`chequeno`,`cheque_status` FROM `fees` where `sadmission`='$AdmissionNo' and `FeesType`='Hostel' order by `quarter`,`FinancialYear` desc";
	$rs = mysql_query($ssql);				
}
?>
...
function Validate(SubmitType)
{
	...
	document.getElementById("frmFees").action="FeesReceiptHostel.php";
...
<tr>
		...
		<td style="width: 19%; height: 37px" class="auto-style24">
			...
			<input name="txtAdjustedLateDays" id="txtAdjustedLateDays" type="text" onkeyup="Javascript:CalculateLateFee();" class="text-box" value="<?php echo $LateDays; ?>"><span class="auto-style37">
			</span>
		</td>
...
<tr>
	...
	<tr>
		...
		<input name="txtTotalAmtPaying" id="txtTotalAmtPaying" type="text" class="text-box" onkeyup="Javascript:CalculateBalance();" value="<?php echo $TotalPaybleAmount;?>" ></td>
...
<tr>			
		...
		<strong>Cheque Status</strong></td>
		...
		<strong>Print Reciept</strong></td>
			...
			<?php
$rsAdvance=mysql_query("select sum(amount) from `fees_student` where `sadmission`='$AdmissionNo' and `feeshead`='HOSTEL AMOUNT CREDIT'");
