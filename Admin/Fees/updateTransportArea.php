...
<?php
$id=$_REQUEST['srno'];
$ssql="SELECT * FROM `tbl_transport_area` WHERE srno='$id'";
$rs= mysql_query($ssql);
...
if($_REQUEST["submit"] !="")
{
  ...
  $transportHead=$_POST["transportHeadID"];
  $transportArea=$_POST["transportArea"];
  
  $status=$_POST["status"];
 
    $sqlUpdate=  mysql_query("UPDATE tbl_transport_area SET transportHeadID='".$transportHead."',transportArea='".$transportArea."',"
            . "status='".$status."' WHERE srno='".$id."'");
   $url="TransportAreaFeesMaster.php";   
 ?>
...
<table width="100%" cellspacing="0" bordercolor="#000000" id="table_11" class="auto-style20" style="height: 33px">
	...
	<tr>
		...
		<strong>Update Transport Area</strong><hr class="auto-style3" style="height: 0px" noshade="noshade">
...
<td><font face="Cambria"><select name="transportHeadID" id="transportHeadID" style="float: left" ; required  class="text-box">
						 <option selected="" value="Select One">Select One</option>
						 <?php
							$ssqlName="SELECT * FROM `tbl_transport_master`WHERE status=1";
							$rsName= mysql_query($ssqlName);
							
							while($row1 = mysql_fetch_assoc($rsName))
							{
									$transportHeadId=$row1['srno'];
                                                                        $transportHead=$row1['transportHead'];
							?>
                                                 <option value="<?php echo $transportHeadId; ?>" <?php if($transportHeadId==$resulrData['transportHeadID']){echo "selected"; } ?>><?php echo $transportHead; ?></option>
...
<td><font face="Cambria"><input type="text" name="transportArea" id="transportArea" size="20" value="<?php echo $resulrData['transportArea']; ?>"  class="text-box"></font></td>
