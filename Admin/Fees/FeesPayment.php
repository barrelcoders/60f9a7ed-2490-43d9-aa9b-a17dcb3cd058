...
if ($_REQUEST["txtAdmissionNo"] != "")
{
	...
	while($rowM=mysql_fetch_row($rsMClass))
	{
		...
		$sqlQ1 = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime` FROM `fees` where  `sadmission`='$AdmissionNo' and `quarter`='Q1' and `FinancialYear`='$CurrentFinancialYear'";
		...
		$sqlQ2 = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime` FROM `fees` where  `sadmission`='$AdmissionNo' and `quarter`='Q2'  and `FinancialYear`='$CurrentFinancialYear'";
		...
		$sqlQ3 = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime` FROM `fees` where  `sadmission`='$AdmissionNo' and `quarter`='Q3'  and `FinancialYear`='$CurrentFinancialYear'";
		...
		$sqlQ4 = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime` FROM `fees` where  `sadmission`='$AdmissionNo' and `quarter`='Q4'  and `FinancialYear`='$CurrentFinancialYear'";
...
//-------------------
			...
			$sql = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime`,`FinancialYear`,`BalanceAmt` FROM `fees` where  `sadmission`='$AdmissionNo' and `FinancialYear`='$CurrentFinancialYear' ORDER BY `quarter` DESC";
		...
		//If Student is Regular ant not paid in current financial year********
			...
			if (mysql_num_rows($rs1) == 0 && $StudentType == "Old Student")
			{
							...
							$sql = "SELECT `fees_amount` , `amountpaid` , `quarter`,`status`,`receipt`,`datetime`,`FinancialYear`,`BalanceAmt` FROM `fees` where  `sadmission`='$AdmissionNo' and `FinancialYear`='$FeeSubmissionFinancialYear' ORDER BY `quarter` DESC";				
							$rs = mysql_query($sql);
							if (mysql_num_rows($rs) > 0)
							{
								while($row = mysql_fetch_row($rs))
		...
		$rsYearlyAmount=mysql_query($ssql);
		while($rowA=mysql_fetch_row($rsYearlyAmount))
		{
		...
		$rsAdvanceAmt=mysql_query("select sum(`amount`) from `fees_advances` where `sadmission`='$AdmissionNo' and `financialyear`='$FeeSubmissionFinancialYear'");
		...
		echo echo "Yearly Total Fee:".$YearlyTotalFee."<br>Paid Amount:".$FeePaidAmount."<br>Advances:".$FeePaidAdvance;
				exit();
		...
		//////HOSTEL CHARGE IS COMMENTED DUE TO IT IS SEPERATLY SUBMITTED
		 ...
		 if ($Hostel=="Yes")
		 {
			...
			{
				$rsHostelFee= mysql_query("select `amount` From `fees_master` where `feeshead`='hostelfees' and `quarter`='$FeeSubmissionQuarter'");
				while($row = mysql_fetch_row($rsHostelFee))
				{
					$HostelCharge=$row[0];
					break;
				}
			...
			if($FeeSubmissionQuarter =="Q4")
			{
				$rsTotalHostelFee=mysql_query("select sum(`amount`) From `fees_student` where `FeesType`='Hostel' and `sadmission`='$AdmissionNo'");
				while($row = mysql_fetch_row($rsTotalHostelFee))
				{
					$YearlyTotalHostelAmt=$row[0];
					break;
				}
				$rsHostelPaidAmt=mysql_query("select sum(`HostelFees`) from `fees_transaction` where `sadmission`='$AdmissionNo' and `FinancialYear`='$FeeSubmissionFinancialYear'");
...
//Calculate Security Charges ans Lab Charges
	...
	{
		$rsSecurity = mysql_query("select `amount` from `fees_master` where `class`='$class' and `feeshead`='Security Fees'  and `financialyear`='$FeeSubmissionFinancialYear'");
		...
		$rsLab = mysql_query("select `amount` from `fees_master` where `class`='$class' and `feeshead`='Lab Charges'  and `financialyear`='$FeeSubmissionFinancialYear'");
...
<img height="30" src="../images/BackButton.png" width="70" style="float: right"></a></p>
	...
	<tr>
		...
		<input name="txtChequeNo" id="txtChequeNo" type="text" class="text-box" readonly="readonly" ></strong></td>
		...
		-->
		<select name="cboBank" id="cboBank" disabled="true" class="text-box">
...
<tr>			
		...
		<td style="height: 29px;" class="style10" colspan="9">
...
<tr>			
		...
		<td style="width: 147px; height: 38px" class="style2" align="center">

		Financial Year</td>


		<td class="auto-style22" style="height: 38px" colspan="2" align="center">
		...
		<td class="style22" style="height: 38px" align="center">
		Adjusted Delay Days:</td>
		

		<td class="style22" style="height: 38px" align="center">
		Late Fees Charges to be paid</td>
...
<tr>			
		...
		<td style="width: 147px; height: 38px" class="style2">
		...
		&nbsp;<span class="style9"></span><input type="text" name="cboQuarter" id="cboQuarter" class="text-box" value="<?php echo $FeeSubmissionQuarter; ?>" readonly="readonly" size="19"></td>


		<td class="style22" style="height: 38px">
		<?php 
		...
		<input name="txtTuition" id="txtTuition" type="text" class="text-box" readonly="readonly" value="<?php echo $TutionFee; ?>" ></td>
		

		<td class="style22" style="height: 38px">

			<input name="txtLateDays" id="txtLateDays" type="text" readonly="readonly" class="text-box" value="<?php echo $LateDays; ?>"></td>
		...
		<td class="style22" style="height: 38px">
			...
			<input name="txtAdjustedLateDays" id="txtAdjustedLateDays" type="text" onkeyup="Javascript:CalculateLateFee();" class="text-box" value="<?php echo $LateDays; ?>"></td>
		...
		<input name="txtAdjustedLateFee" id="txtAdjustedLateFee" type="text" class="text-box" value="<?php echo $LateFee; ?>"></td>
...
<tr>			
		...
		<td class="auto-style22" style="height: 38px" colspan="8">
		<!--
		...
		-->
		&nbsp;<textarea name="txtRemarks" id="txtRemarks" rows="2" class="text-box-address" cols="20"></textarea></td>
...
<tr>			
		...
		<td style="width: 147px; " class="style2">

		<blockquote>

		FeesPayable</blockquote>
		</td>


		<td style="width: 50px; " class="auto-style22" colspan="8">

		

		<input name="txtTotal" id="txtTotal" type="text" class="text-box" readonly="readonly"></td>
...
-->		
<tr>
		<td style="width: 147px; height: 37px" class="style8">

		<blockquote>

		Total Fees Paid</blockquote>
		</td>

		<td style="width: 147px; height: 37px" class="auto-style24">



		<input name="txtTotalAmtPaying" id="txtTotalAmtPaying" type="text" class="text-box" onkeyup="Javascript:CalculateBalance();"></td>

		<td style="width: 8px; height: 37px; text-align:left" class="style18" colspan="4">



		<strong>Balance</strong></td>

		<td style="width: 7px; height: 37px" class="auto-style24" colspan="3">

			<span class="auto-style37">
			&nbsp;</span><input name="txtBalance" id="txtBalance" type="text" class="text-box"></td>

	</tr>
		
<tr>
		<td style="height: 37px" class="style8" colspan="9">

		<blockquote>

		&nbsp;</blockquote>
		<p style="text-align: center"><span class="auto-style37">
			&nbsp;</span><strong><input name="BtnSubmit" type="button" value="Preview" onclick="Javascript:Validate('Preview');" class="text-box">&nbsp;&nbsp;&nbsp;&nbsp;
		<input name="BtnSubmit1" type="button" value="Submit" onclick="Javascript:Validate('Final');" class="text-box"></strong></td>
