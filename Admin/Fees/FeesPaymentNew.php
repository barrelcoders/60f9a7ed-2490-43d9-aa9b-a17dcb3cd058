...
while($row1 = mysql_fetch_row($rs))
{
	...
	$TotalConcessionAmt=0;
		...
		$rsCheckQ1=mysql_query("SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `feeshead`='Fees First Installment'");
			...
			{
				//$rsFeePaidQ1=mysql_query("SELECT `amount` FROM `fees_master` WHERE `class`='$MasterClass' and `quarter`='Q1' and `financialyear`='$CurrentFinancialYear' and `StudentType`='$QStudentType' and `feeshead`='TUITION FEES'");
			...
			else
			{
				...
				{
					$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date` from `fees` where `sadmission`='$sadmission' and `quarter`='Q1' and `FeesType`='Regular' and cheque_status !='Bounce'");
		...
		//CHECK FEE SUBMISSION IN FEE TABLE FOR QUARTER Q2 IF FOUND THEN SHOW RECEIPT DETAIL OTHER WISE TO PAID AMOUNT WILL BE SHOWN//////////////////
			...
			else
			{
				...
				{
					$rsRcptNo=mysql_query("select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date`  from `fees` where `sadmission`='$sadmission' and `quarter`='Q2' and `FeesType`='Regular' and cheque_status !='Bounce'");
					while($rowRcpt=mysql_fetch_row($rsRcptNo))
					{
		...
		while($rowFPQ4=mysql_fetch_row($rsTotalHostelAmt))
		{
			...
			$rsCheckQ4=mysql_query("SELECT sum(`amount`) FROM `fees_student` WHERE `sadmission`='$sadmission' and `financialyear`='$CurrentFinancialYear' and `feeshead`='Total Bill Amount'");
...
//-------------------- Previous Payment history----------------------------------------------------------
	$ssql = "SELECT `quarter`,`fees_amount`,(`amountpaid`-`AdjustedLateFee`) as `amountpaid`,`BalanceAmt`,`chequeno`,`cheque_status`,`receipt`,date_format(`date`,'%d-%m-%Y') as `date`,`FinancialYear`,`AdjustedLateFee`,`cheque_bounce_amt` FROM `fees` where `sadmission`='$AdmissionNo' and `FeesType`='Regular' order by `quarter`,`FinancialYear` desc";
...
<tr>			
		...
		<select name="cboFinancialYear" id="cboFinancialYear" onchange="javascript:CheckFinancialYear();" style="height: 22px">
				...
				while($rowFY = mysql_fetch_row($rsFY))
				{
							...
							$FYear=$rowFY[1];
				?>
				<option value="<?php echo $Year; ?>" <?php if($CurrentFinancialYear==$Year) { echo "selected"; } ?>><?php echo $FYear; ?></option>
				<?php 
				}
				?>
		</select></td>
...
<?php
$rsTotalBill=mysql_query("select `amount` from `fees_student` where `sadmission`='$AdmissionNo' and `financialyear`='$CurrentFinancialYear' and `feeshead`='TOTAL BILL AMOUNT'");
$row1=mysql_fetch_row($rsTotalBill);
$BillAmount=$row1[0];
$rsAdvance=mysql_query("select sum(amount) from `fees_student` where `sadmission`='$AdmissionNo' and `financialyear`='$CurrentFinancialYear' and `feeshead`='Advances'");
...
?>		
	...
	<?php
	$ssql="select distinct `feeshead`,`amount`,(select `sname` from `student_master` where `sadmission`=a.`sadmission`) as `sname`,(select `DiscontType` from `student_master` where `sadmission`=a.`sadmission`) as `DiscountType`,`FeesType` from `fees_student` as `a` where `sadmission`='$txtAdmissionNo' and `financialyear`='$CurrentFinancialYear' and `feeshead`!='TOTAL BILL AMOUNT'  and `feeshead` not like '%INSTAL%'";
	
	$rs= mysql_query($ssql);
	
	$ssql1="select `amount`,(select `sname` from `student_master` where `sadmission`=a.`sadmission`) as `sname`,(select `DiscontType` from `student_master` where `sadmission`=a.`sadmission`) as `DiscountType`,`class` from `fees_student` as `a` where `sadmission`='$txtAdmissionNo' and `financialyear`='$CurrentFinancialYear' and `feeshead`='TOTAL BILL AMOUNT'";
...
<td height="22" style="width: 641px" class="style2" >
	...
	<?php 
	$ssql="select distinct `feeshead`,`amount`,(select `sname` from `student_master` where `sadmission`=a.`sadmission`) as `sname`,(select `DiscontType` from `student_master` where `sadmission`=a.`sadmission`) as `DiscountType`,`FeesType` from `fees_student` as `a` where `sadmission`='$txtAdmissionNo' and `financialyear`='$CurrentFinancialYear' and `feeshead`!='TOTAL BILL AMOUNT' and `FeesType` in ('Regular') and `feeshead`  like '%INSTAL%'";
	...
	<?php
	$ssql="select distinct `feeshead`,`amount`,(select `sname` from `student_master` where `sadmission`=a.`sadmission`) as `sname`,(select `DiscontType` from `student_master` where `sadmission`=a.`sadmission`) as `DiscountType`,`FeesType` from `fees_student` as `a` where `sadmission`='$txtAdmissionNo' and `financialyear`='$CurrentFinancialYear' and `feeshead`!='TOTAL BILL AMOUNT'  and `feeshead`  like '%INSTAL%'";
