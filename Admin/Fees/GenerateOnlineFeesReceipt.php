...
<?php
	...
	//$rsPending=mysql_query("select `TxnId` from `fees` where `PaymentMode`='Online'");
	$rsPending=mysql_query("select `TxnId` from `fees` where `PaymentMode`='Online' and `Quarter`='Q4' and `receipt` in (select `ReceiptNo` from `fees_receipt_code` where `ReceiptCode`='')");
	...
	{		
		...
		while($rows = mysql_fetch_row($rsStudentDetail))
		{
			...
			break;
				...
				$rsDetail=mysql_query("select `sadmission` ,`sname` ,`sclass`,`srollno`,`fees_amount`,`amountpaid`,`BalanceAmt`,`quarter`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date`,`FinancialYear`,`status`,`receipt`,`finalamount`,`ReceiptFileName`,`PaymentMode`,`chequeno`,`bankname`,`cheque_date`,`cheque_status`,`ActualLateFee`,`ActualDelayDays`,`AdjustedLateFee`,`AdjustedDelayDays`,`Remarks`,`FeesType`,`TxnAmount`,`TxnId`,`TxnStatus`,`PGTxnId` from `fees` where `TxnId`='$txnid'");
