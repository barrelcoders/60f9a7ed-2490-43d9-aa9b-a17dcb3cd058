...
if ($_REQUEST["txtAdmissionNo"] != "")
{
		...
		$sqlStudentDetail= "SELECT `sname`,`sclass`,`srollno`,`FinancialYear`,`previous_sclass`,`Hostel`,`DiscontType` ,`sfathername`,`MotherName`,`routeno`,`MasterClass` FROM `student_master` where  `sadmission`='$AdmissionNo'";
	...
	{
	$rsFeeHead=mysql_query("select distinct `feeshead`,sum(`amount`),sum(`actualamount`),sum(`discountamount`) from `fees_student1` where `sadmission`='$AdmissionNo' and `financialyear`='$CurrentFinancialYear' and `feeshead` not in ('COMPUTER FEES','SMART CLASS','SCIENCE FEES','ANNUAL CHARGES','PROVISIONAL AMOUNT') and `FeesType`='Regular' group by `feeshead`");
	}
	else
	{
		$rsFeeHead=mysql_query("select distinct `feeshead`,sum(`amount`),sum(`actualamount`),sum(`discountamount`) from `fees_student1` where `sadmission`='$AdmissionNo' and `financialyear`='$CurrentFinancialYear' and `feeshead` not in ('COMPUTER FEES','SMART CLASS','SCIENCE FEES','ANNUAL CHARGES') and `FeesType`='Regular' group by `feeshead`");
...
$ssqlAdmissionFeePaymentDetail="SELECT `RegistrationNo`, `RegistrationDate`, `AdmissionFeeReceipt`, `sadmission`, `AdmissionDate`, `sname`, `slastname`, `FinancialYear`, `TxnAmount`, `TxnId`, `TxnStatus`, `PGTxnId`,  `PaymentMode`, `BankName`, `ChequeDate`, `ChequeBounceAmt`, `ChequeStatus` FROM `NewStudentAdmission`  WHERE `sadmission`='$AdmissionNo'  ";

$AdmissionFeePaymentDetail=mysql_query($ssqlAdmissionFeePaymentDetail);
...
?>
<?php
while($row1 = mysql_fetch_row($AdmissionFeePaymentDetail))
	{
       $AdmissionRegistrationNo=$row1[0];
       $AdmissionRegistrationDate=$row1[1]; 
       $AdmissionFeeReceipt=$row1[2];
       $Admissionsadmission=$row1[3];
       $AdmissionDate=$row1[4];
       $Admissionsname=$row1[5];
       $Admissionslastname=$row1[6];
       $AdmissionFinancialYear=$row1[7];
       $AdmissionTxnAmount=$row1[8];
       $AdmissionTxnId=$row1[9];
       $AdmissionTxnStatus=$row1[10];
       $AdmissionPGTxnId=$row1[11];
       $AdmissionPaymentMode=$row1[12];
       $AdmissionBankName=$row1[13];
       $AdmissionChequeDate=$row1[14];
       $AdmissionChequeBounceAmt=$row1[15];
       $AdmissionChequeStatus=$row1[16];
...
<tr>			
		...
		<?php echo $Admissionsadmission; ?></td>


		<td style="width: 100px; height: 20px;" class="style12">

		<?php echo $Admissionsname; ?>&nbsp;&nbsp;<?php echo $Admissionslastname; ?> </td>
		...
		<?php echo $AdmissionFeeReceipt; ?></td>


		<td style="width: 100px; height: 20px;" class="style12">

		<?php echo $AdmissionTxnAmount; ?></td>
		...
		<?php echo $AdmissionChequeBounceAmt; ?></td>
		...
		<?php echo $AdmissionPaymentMode; ?></td>
		...
		<?php echo $AdmissionTxnId; ?></td>


		<td style="width: 101px; height: 20px;" class="style17">

		<?php echo $AdmissionChequeStatus; ?></td>


		<td style="width: 101px; height: 20px;" class="style17">

		<?php echo $AdmissionDate; ?></td>
