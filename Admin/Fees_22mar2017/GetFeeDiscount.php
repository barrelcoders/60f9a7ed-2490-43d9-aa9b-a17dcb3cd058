...
if ($discountreason == "tuitionfees")
{
	$ssqlFY1 = "SELECT `percentage` FROM `discounttable` where `head`='$discounttype' and `financialyear`='$financialyear' and `discounttype`='tuitionfees'";
	$rsFY1= mysql_query($ssqlFY1);
	if (mysql_num_rows($rsFY1) > 0)
	{
		while($row = mysql_fetch_row($rsFY1))
...
if($discountreason == "hostelfees")
{
	$ssqlFY1 = "SELECT `percentage` FROM `discounttable` where `head`='$discounttype' and `financialyear`='$financialyear' and `discounttype`='Hostel'";
	$rsFY1= mysql_query($ssqlFY1);
	if (mysql_num_rows($rsFY1) > 0)
	{
		while($row = mysql_fetch_row($rsFY1))
