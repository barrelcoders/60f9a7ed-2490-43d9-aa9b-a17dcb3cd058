...
//{
	...
	$sstr="select * from `fees` where `sadmission`='$AdmissionNo' and `quarter`='$Quarter' and `FinancialYear`='$FinancialYear' and `FeesType`='Regular' and `cheque_status` !='Bounce'";
	$rs = mysql_query($sstr);
	...
	$ssql2="INSERT INTO `fees_student1` (`sadmission`,`class`,`Name`,`feeshead`,`amount`,`financialyear`,`FeesType`) VALUES ('$AdmissionNo','$Class','$Name','$InstallmentName','$FinalInstAmountPaid','$FinancialYear','')";
	mysql_query($ssql2) or die(mysql_error());
//}

//Previous Payment History***
$ssqlFeePaymentDetail="SELECT `srno`,`sadmission`, `sname`, `sclass`, `srollno`, `fees_amount`, `InstallmentAmount`, `ActualLateFee`, `AdjustedLateFee`, `cheque_bounce_amt`, `finalamount`, `amountpaid`, `BalanceAmt`, `quarter`, `FinancialYear`, `status`, `receipt`, `date`, `datetime`, `refundamount`, `refunddate`, `cancelamount`, `canceldate`, `ReceiptFileName`, `FeeReceiptCode`, `PaymentMode`, `chequeno`, `cheque_date`, `bankname`, `cheque_status`, `ActualDelayDays`, `AdjustedDelayDays`, `Remarks`, `FeesType`, `SendToBank`, `TxnAmount`, `TxnId`, `TxnStatus`, `PGTxnId` FROM `fees` WHERE   `sadmission`='$AdmissionNo'";
