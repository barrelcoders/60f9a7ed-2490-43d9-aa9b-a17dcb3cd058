
   <link rel="stylesheet" href="../../css/bootstrap.min.css" />
   <link rel="stylesheet" href="../../css/payroll.css" />
   <script src="../../js/bootstrap.min.js"></script>

<!----Header--------->
 <div class="row t">
  <div class="col-md-5"><img src="../../images/logo.png" class="img-responsive"></div>
  <div class="col-md-5 o"><!--<img src="OSP.png" class="img-responsive">--></div>
  <div class="col-md-1" align="right"><span class="name">ABC</span></div>
  <div class="col-md-1">
  		<span class="imghover"><img src="../../images/DPS Logo.jpg" class="img-circle" width="50px" height="50px">
             <ul class="submanu">
               <li class="te"></li>
               <li><a href="../ChangePassword.php"><button class="btnmanu">Change Password</button></a></li>
               <li><a href="../../TeacherLogin.php"><button class="btnmanu">Logout</button></a></li>
             </ul>
        </span>
  </div> 
 </div> 
 <!-----manu---->
 <div>
   <nav id="nav" role="navigation">
    <a href="#nav" title="Show navigation">Show Manu</a>
    <a href="#" title="Hide navigation">Hide Manu</a>
    <ul>
        <li class="m"><a href="../index.php" active><button class="btnmanu">HOME</button></a></li>
        <li class="m "><a href="../Hris/HRISmydetail.php"><button class="btnmanu">HRIS </button></a></li>
        <li class="m "><a href="../Leave/NewLeave.php"><button class="btnmanu">LEAVE</button></a></li>
        <li  class="m1"><a href="../AttendanceMyAttendance.php" ><button class="btnmanu">ATTENDANCE</button></a></li>
		<li  class="m "><a href="../Payroll/payroll.php" ><button class="btnmanu">PAYROLL</button></a></li>
		<li class="m"><a href="../Exit/exit.php"><button class="btnmanu">EXIT</button></a></li>
		<li  class="m1 active"><a href="helpdeskNewquery.php"><button class="btnmanu">HELPDESK</button></a></li>    
    </ul>
</nav>
  
</div>
